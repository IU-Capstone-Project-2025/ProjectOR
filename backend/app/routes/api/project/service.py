...
class ProjectService:
    ...
    async def delete_project(self, project_id: int, user: UserInDB) -> MessageSchema:
        project = await self.data_access.get_project_by_id(project_id)
        if project is None:
            raise HTTPException(
                status_code=404, detail=f"Project with ID {project_id} not found."
        ...
        return MessageSchema(
            msg=f"Project {project_id} was successfully deleted", success=res
        )
    ...
    async def cancel_application(self, project_id, user: UserInDB) -> MessageSchema:
        application = await self.data_access.get_application_by_user_and_project_id(
            project_id, user.id
        )
        if application is None:
        ...
        if application.is_approved:
            raise HTTPException(status_code=400, detail=f"Application was approved")
        res = await self.data_access.cancel_application(project_id, user.id)
        return MessageSchema(msg=f"Application was successfully deleted", success=res)
